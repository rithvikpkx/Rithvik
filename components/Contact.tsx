...
function DockIcon({
  ...
  // Distance from mouse to icon center drives the scale
  ...
  const scaleVal = useTransform(distance, [-110, 0, 110], [1, 1.55, 1]);
  const scale = useSpring(scaleVal, { mass: 0.1, stiffness: 220, damping: 14 });
  ...
  if (onClick) {
    return (
      <button className="dock-btn" onClick={onClick} aria-label={label}>
        {inner}
      </button>
    );
  }
